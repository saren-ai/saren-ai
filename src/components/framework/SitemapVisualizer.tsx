    ...
    return (
        <div className="w-full bg-white dark:bg-offblack border-b border-slate/20 dark:border-charcoal/10 relative transition-colors duration-300">
            ...
            <div
                ...
                className="overflow-x-auto pb-8 pt-8 px-4 scrollbar-thin scrollbar-thumb-slate dark:scrollbar-thumb-charcoal/10 scrollbar-track-transparent flex gap-8 min-w-full justify-center"
            >
                {/* Connecting Line (Horizontal Spine) */}
                <div className="absolute top-[60px] left-10 right-10 h-0.5 bg-slate/20 dark:bg-charcoal/10 -z-0" />
                        ...
                        <div key={phase} className="flex-shrink-0 w-48 relative z-10 flex flex-col items-center">
                            ...
                            <div className={`
                                ...
                                ${isSelectedPhase
                                    ...
                                    : "bg-white dark:bg-offblack border-slate/20 dark:border-charcoal/10"}
                            `}>
                                <div className={`text-[10px] font-mono uppercase tracking-widest opacity-60 mb-1 ${isSelectedPhase ? "text-charcoal dark:text-white" : "text-slate dark:text-slate"}`}>Phase 0{index + 1}</div>
                                <h3 className={`text-lg font-bold ${isSelectedPhase ? colors.text : "text-black dark:text-white"}`}>{phase}</h3>

                                {/* Connector Dot Bottom */}
                                <div className="absolute -bottom-[25px] left-1/2 -translate-x-1/2 w-0.5 h-6 bg-slate/20 dark:bg-charcoal/10" />
                            </div>
                                ...
                                {phasePrompts.map((prompt, pIndex) => {
                                        ...
                                        <div className="flex items-start gap-3">
                                            <span className={`
                                                ...
                                                ${isSelected
                                                    ...
                                                    : "bg-ash dark:bg-charcoal/10 text-slate dark:text-white/30"}
                                            `}>
                                                {prompt.id}
                                            </span>
                                            <span className={`text-xs font-medium leading-snug ${isSelected ? colors.text : "text-charcoal dark:text-white/90"}`}>
                                                {prompt.title}
                                            </span>
                                        </div>
                                    );
                                    ...
                                    const className = `
                                        ...
                                        ${isSelected
                                            ...
                                            : "bg-white dark:bg-charcoal text-charcoal dark:text-ash/70 border-slate/20 dark:border-charcoal/10 hover:border-slate/40 dark:hover:border-charcoal/20 hover:bg-ash dark:hover:bg-charcoal/5"}
                                    `;
