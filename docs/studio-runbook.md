...
> **What Studio is now:** a pipeline cockpit. `/studio` opens directly on the
...
| Route | What it is |
|---|---|
| `/studio` | **The pipeline.** Funnel strip + Do-Next queue + gamification. The landing page. |
| `/studio/contacts/[id]` | Per-contact cockpit — inline field edit, sequence/touch timeline, reply logging |
| `/studio/login` | Magic-link sign-in |

There is no longer a `/studio/contacts` list, `/studio/sequences`, or
`/studio/outreach-pages`. Every contact is reachable from the queue.
...
run via Claude); outreach **state** is the `touches` event log. The studio reads
`v_pipeline` and writes `touches`. Full field list: `hustle-flow-schema-reference.md`.

---

## The pipeline view — `/studio`
...
- **Review** → opens the contact cockpit (`/studio/contacts/[id]`) to read the copy
...
sourcing skills (Apollo enrich / `/sales outreach`), not in the UI. The studio logs
and reviews outreach; it never auto-sends.

---

## The contact cockpit — `/studio/contacts/[id]`
...
- **Does not send email.** Sending happens in Gmail/Apollo/LinkedIn; the studio logs the touch.
